            ...
            <div id="page-wrapper">
	                    ...
	                    <div class="panel panel-default">
	                        ...
	                        <div class="panel-body">
	                            <div class="dataTable_wrapper">
                                    ...
                                    // printf ("<br>".testprintf_query_2($result));
                                    ?>
                ...
                <div class="container-fluid">
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            <h1 class="page-header">PHP MySQLi 筆記</h1>
                            <br><?php 
                            testPageSeparate()
...
?><br>
<?php /*function 打包程式 印出mysqli_query    DataTables 範例*/
    function testprintf_query_2( $result ) {
        if (!$rowcount=mysqli_num_rows($result)){
            return "null";
        }
            $StrAll = "<table id='example' class='display' cellspacing='0' width='100%'>";

            $fieldinfo=mysqli_fetch_fields($result);


                $StrAll = $StrAll."<thead><tr>";
            foreach ($fieldinfo as $val){
                $StrAll = $StrAll."<th>". $val->name."</th>";
            }
                $StrAll = $StrAll."</tr></thead>";


            for($i = 0;$i< $rowcount;$i++ ){
                if (!mysqli_data_seek ($result, $i)) { printf ("Cannot seek to row %d\n", $i); continue;}
                if(!($row = mysqli_fetch_array ($result))) {continue;}
                $StrAll = $StrAll."<tbody><tr>";
                    foreach ($fieldinfo as $val){
                        $StrAll = $StrAll."<td>". $row[$val->name]."</td>";
                    }
                $StrAll = $StrAll."</tr></tbody>";
            }
            $StrAll = $StrAll."</table>";
            return $StrAll;
    } 
...
<?php /*function 打包程式  舊的 未包裝的 */
    function test20160128( $con, $sql ) {
        if ($result=mysqli_query($con,$sql))
        {
            $rowcount=mysqli_num_rows($result);
        }
